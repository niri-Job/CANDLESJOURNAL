...
// ─── SVG icon wrapper ─────────────────────────────────────────────────────────
function Ico({ children, size = 24, color = "#d4a017", style }: {
...
// Social SVGs
const IcoTwitterX = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#d4a017" strokeWidth="1.5" strokeLinecap="round">
    <path d="M4 4l16 16M20 4 4 20"/>
  </svg>
);
const IcoTelegram = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#d4a017" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const IcoLock = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#d4a017" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ display: "inline", verticalAlign: "middle", marginRight: 6 }}>
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(212,160,23,0.12)" }}>
      <button onClick={() => setOpen(!open)} style={{
        ...
        <span>{q}</span>
        <span style={{ color: "#d4a017", fontSize: "1.375rem", flexShrink: 0, transition: "transform 0.25s", transform: open ? "rotate(45deg)" : "none" }}>+</span>
      </button>
      <div style={{
...
function CmpRow({ label, niri, other, highlight }: { label: string; niri: string; other: string; highlight?: boolean }) {
  return (
    <tr style={{ background: highlight ? "rgba(245,197,24,0.05)" : "transparent" }}>
      <td style={{ padding: "0.875rem 1rem", color: "var(--cj-text-muted)", fontSize: "0.9rem", borderBottom: "1px solid rgba(212,160,23,0.10)" }}>{label}</td>
      <td style={{ padding: "0.875rem 1rem", textAlign: "center", borderBottom: "1px solid rgba(212,160,23,0.10)", borderLeft: "2px solid rgba(212,160,23,0.35)" }}>
        <span style={{ color: "var(--cj-gold)", fontWeight: 700, fontSize: "0.9rem" }}>{niri}</span>
      </td>
      <td style={{ padding: "0.875rem 1rem", textAlign: "center", borderBottom: "1px solid rgba(212,160,23,0.10)" }}>
...
function CheckItem({ text, dim }: { text: string; dim?: boolean }) {
  return (
    <div style={{ display: "flex", gap: "0.625rem", alignItems: "flex-start", marginBottom: "0.75rem" }}>
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#d4a017" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 2 }}>
...
export default function LandingPage() {
  ...
  const heroCards = [
    ...
    {
      accent: "#d4a017",
      icon: <IcoLightbulb color="#d4a017" />,
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .shimmer-text {
          background: linear-gradient(90deg,#d4a017 0%,#f5c842 40%,#d4a017 60%,#b8860b 100%);
        ...
        .gold-btn {
          ...
          color: #0d0d12; border: none; border-radius: 0.5rem;
          font-weight: 700; cursor: pointer;
          transition: transform 0.2s, box-shadow 0.2s;
        }
        .gold-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 32px rgba(212,160,23,0.45); }
        .outline-btn {
        ...
        .outline-btn:hover { background: rgba(212,160,23,0.10); border-color: #d4a017; }
      ...
      <nav className={scrolled ? "lp-nav-scrolled" : ""} style={{
        ...
        <Link href="/" style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: "0.625rem" }}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" style={{ width: 32, height: 32, flexShrink: 0, filter: "drop-shadow(0 0 8px rgba(212,160,23,0.30))" }}>
            ...
            <path d="M52 54 L56 47 L60 56 L64 44 L68 50" fill="none" stroke="#d4a017" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
            <circle cx="68" cy="50" r="2.5" fill="#d4a017"/>
          </svg>
            ...
            {([
              ...
              <button key={key} title={title} onClick={() => setThemeAndSave(key)}
                style={{
                  ...
                  background: theme === key ? "rgba(212,160,23,0.30)" : "rgba(255,255,255,0.05)",
                  border: `1px solid ${theme === key ? "rgba(212,160,23,0.55)" : "rgba(255,255,255,0.12)"}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: theme === key ? "#d4a017" : "#888", transition: "all 0.2s",
                }}
              >{svg}</button>
            ))}
          </div>
        ...
        <button className="hamburger" onClick={() => setMenuOpen(!menuOpen)}
          ...
          <div style={{ width: 22, height: 2, background: "#d4a017", borderRadius: 2, transition: "transform 0.2s", transform: menuOpen ? "rotate(45deg) translate(5px,5px)" : "none" }} />
          <div style={{ width: 22, height: 2, background: "#d4a017", borderRadius: 2, opacity: menuOpen ? 0 : 1, transition: "opacity 0.2s" }} />
          <div style={{ width: 22, height: 2, background: "#d4a017", borderRadius: 2, transition: "transform 0.2s", transform: menuOpen ? "rotate(-45deg) translate(5px,-5px)" : "none" }} />
        </button>
      </nav>
          ...
          {([
            ...
            <button key={key} onClick={() => { setThemeAndSave(key); closeMenu(); }}
              style={{
                ...
                background: theme === key ? "rgba(212,160,23,0.30)" : "rgba(255,255,255,0.05)",
                border: `1px solid ${theme === key ? "rgba(212,160,23,0.55)" : "rgba(255,255,255,0.1)"}`,
                color: theme === key ? "#d4a017" : "#888", fontSize: "0.8125rem", fontWeight: 600,
              }}
            >{label}</button>
          ))}
        </div>
      ...
      <section style={{
        ...
        background: [
          "radial-gradient(ellipse 110% 85% at -8% 95%, rgba(212,160,23,0.42) 0%, rgba(212,160,23,0.16) 32%, transparent 60%)",
          "radial-gradient(ellipse 55% 40% at 105% 5%,  rgba(212,160,23,0.10) 0%, transparent 50%)",
          "var(--cj-bg)",
        ].join(", "),
        ...
        <div style={{ maxWidth: 1140, margin: "0 auto", padding: "5rem 1.5rem 4rem", width: "100%" }}>
          <div ref={heroRef} className="fade-up hero-cols" style={{ display: "flex", alignItems: "center", gap: "4rem", justifyContent: "space-between" }}>
            ...
            <div style={{ flex: "0 0 400px", position: "relative" }}>
              ...
              <div style={{
                position: "absolute", bottom: -36, left: "12%", right: "12%", height: 70,
                background: "radial-gradient(ellipse at center, rgba(212,160,23,0.45) 0%, transparent 70%)",
                filter: "blur(18px)", pointerEvents: "none",
              }} />
              <div style={{
                background: "linear-gradient(145deg, #1c1a28, #12111e)",
                border: "1px solid rgba(212,160,23,0.3)",
                borderRadius: "1.25rem", overflow: "hidden",
                boxShadow: "0 0 0 1px rgba(212,160,23,0.06), 0 32px 80px rgba(0,0,0,0.75), 0 0 70px rgba(212,160,23,0.15)",
                ...
                <div style={{
                  ...
                  <span style={{ fontWeight: 800, color: "#0d0d12", fontSize: "0.75rem", letterSpacing: "0.1em" }}>NIRI DASHBOARD</span>
                  <span style={{ color: "#0d0d12", fontSize: "0.7rem", fontWeight: 700, opacity: 0.75 }}>● LIVE</span>
                </div>
                {/* Stats row */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.5rem", padding: "1rem", borderBottom: "1px solid rgba(212,160,23,0.08)" }}>
                  {([["P&L", "+$2,840", "#4ade80"], ["Win Rate", "67%", "#d4a017"], ["Trades", "247", "#ffffff"]] as const).map(([l, v, c]) => (
                    <div key={l} style={{ textAlign: "center", padding: "0.625rem 0.25rem", background: "rgba(212,160,23,0.04)", borderRadius: "0.5rem" }}>
                ...
                <div style={{ padding: "0.25rem 1rem 0.75rem" }}>
                  ...
                  {[
                    ...
                    <div key={idx} className={`trade-row-${idx + 1}`} style={{
                      display: "flex", justifyContent: "space-between", alignItems: "center",
                      padding: "0.4rem 0",
                      borderBottom: idx < 2 ? "1px solid rgba(212,160,23,0.07)" : "none",
                    }}>
                ...
                <div style={{
                  ...
                  <span style={{ color: "#d4a017", fontSize: "0.725rem", fontWeight: 600, lineHeight: 1.4 }}>AI: XAUUSD London session — your edge at 78% win rate</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{ background: "linear-gradient(90deg,rgba(212,160,23,0.08) 0%,rgba(212,160,23,0.05) 50%,rgba(212,160,23,0.08) 100%)", padding: "1.25rem 1.5rem" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: "2rem" }}>
          {[
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.875rem" }}>
              <span style={{ color: "#d4a017", fontWeight: 900, fontSize: "1.5rem", fontVariantNumeric: "tabular-nums", letterSpacing: "-0.02em" }}>{s.val}</span>
              <span style={{ color: "var(--cj-text-muted)", fontSize: "0.875rem", lineHeight: 1.4, maxWidth: 200 }}>{s.label}</span>
              {i < 2 && <span style={{ color: "rgba(212,160,23,0.25)", fontSize: "1.5rem", display: "block" }}>|</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      <section className="lp-dark-bg" style={{ background: "#0A0A0F", padding: "5rem 1.5rem 6rem" }}>
        <div style={{ maxWidth: 1160, margin: "0 auto", display: "flex", alignItems: "center", gap: "4rem", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "1 1 320px", maxWidth: 460 }}>
            <p style={{ color: "#d4a017", fontSize: "0.8125rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem" }}>
            ...
            {[
              ...
              <div key={f} style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", marginBottom: "0.875rem" }}>
                <span style={{ color: "#d4a017", fontWeight: 700, flexShrink: 0, marginTop: 2 }}>→</span>
          ...
          <div style={{ flex: "1 1 460px" }}>
            ...
            <img
              ...
              style={{
                ...
                borderRadius: 14,
                display: "block",
                boxShadow: "0 0 0 1px rgba(212,160,23,0.30), 0 32px 80px rgba(0,0,0,0.7), 0 0 60px rgba(212,160,23,0.30)",
      ...
      {/* ── PAIN ────────────────────────────────────────────────────────────── */}
      <section id="why" style={{ padding: "7rem 1.5rem", background: "radial-gradient(circle at top left, rgba(212,160,23,0.10) 0%, transparent 50%), var(--cj-bg)", position: "relative" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>

          {/* Brutal truth callout */}
          <div className="fade-up" ref={painRef} style={{
            background: "linear-gradient(135deg,rgba(224,85,85,0.06) 0%,rgba(212,160,23,0.06) 100%)",
          ...
          <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
            {[
              ...
              { ico: <IcoTrendDown color="#b8860b" />, title: "Your results are inconsistent",
                text: "Profitable weeks followed by losing weeks with no clear explanation. NIRI finds the variables, whether session, pair, emotion or timing, that explain the difference.", accent: "#b8860b" },
              ...
              { ico: <IcoHelpCircle color="#b8860b" />, title: "You do not know which setups work",
                text: "Trading multiple strategies without performance data for each one. NIRI segments results by setup type so you can see which approaches are profitable.", accent: "#b8860b" },
            ].map((p, i) => {
              ...
              return (
                <div key={p.title} ref={ref} className="fade-up card-hover lp-card" style={{ transitionDelay: `${i * 80}ms`,
                  background: "linear-gradient(145deg,#120f04,#0d0d12)",
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3.5rem", alignItems: "start" }} className="grid-2">
              ...
              {[
                ...
                <div
                  ...
                  <div style={{
                    ...
                    background: howStep === i ? "linear-gradient(135deg,#d4a017 0%,#f5c842 50%,#d4a017 100%)" : "rgba(212,160,23,0.10)",
                    border: howStep === i ? "none" : "1px solid rgba(212,160,23,0.25)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontWeight: 800, color: howStep === i ? "#0d0d12" : "var(--cj-text-muted)",
            ...
            <div style={{
              ...
              {howStep === 1 && (
                <div style={{ padding: "1.75rem" }}>
                  ...
                  {[
                    ...
                    { label: "XAUUSD Edge Score", pct: 82, col: "#d4a017" },
                    { label: "TP Discipline", pct: 45, col: "#b8860b" },
                  ].map((item) => (
              ...
              {howStep === 2 && (
                <div>
                  <div style={{ background: "linear-gradient(135deg,#d4a017 0%,#f5c842 50%,#d4a017 100%)", padding: "0.875rem 1.5rem", display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontWeight: 800, color: "#0d0d12", fontSize: "0.8125rem", letterSpacing: "0.06em" }}>THIS SESSION&rsquo;S REPORT</span>
                    <span style={{ color: "#0d0d12", fontSize: "0.75rem", opacity: 0.7 }}>May 13, 2026</span>
                  </div>
                  <div style={{ padding: "1.5rem" }}>
                    {[
                      ...
                      { ico: "→", col: "#d4a017", label: "FOCUS NEXT", text: "XAUUSD London session only. Your 78% win rate edge is here." },
                    ].map((row) => (
                      <div key={row.label} style={{ display: "flex", gap: "0.875rem", marginBottom: "1rem", paddingBottom: "1rem", borderBottom: "1px solid rgba(212,160,23,0.08)" }}>
      ...
      <section id="ai-showcase" style={{ padding: "7rem 1.5rem", background: "radial-gradient(circle at top left, rgba(212,160,23,0.10) 0%, transparent 50%), var(--cj-bg)", position: "relative" }}>
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          ...
          <div className="lp-card" style={{
            ...
            <div style={{ background: "linear-gradient(135deg,#d4a017 0%,#f5c842 50%,#d4a017 100%)", padding: "1rem 1.75rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontWeight: 800, color: "#0d0d12", fontSize: "0.9375rem", letterSpacing: "0.06em" }}>COACHING REPORT</span>
              <span style={{ color: "#0d0d12", fontSize: "0.8125rem", opacity: 0.7 }}>Week of April 28, 2026</span>
            </div>
            <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
              {[
                ...
                <div key={row.label} style={{ padding: "1.5rem 1.75rem", borderBottom: "1px solid rgba(212,160,23,0.10)", borderRight: "1px solid rgba(212,160,23,0.10)" }}>
            ...
            <div style={{ padding: "1.5rem 1.75rem", background: "rgba(212,160,23,0.06)", borderTop: "1px solid rgba(212,160,23,0.12)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.625rem" }}>
            ...
            {features.map((f, i) => {
              ...
              return (
                <div key={f.title} ref={fRef} className="fade-up card-hover lp-card" style={{ transitionDelay: `${i * 60}ms`,
                  ...
                  <h3 style={{ color: "#d4a017", fontWeight: 700, fontSize: "1rem", marginBottom: "0.5rem", lineHeight: 1.4 }}>{f.title}</h3>
      ...
      <section style={{ padding: "7rem 1.5rem", background: "radial-gradient(circle at top left, rgba(212,160,23,0.10) 0%, transparent 50%), var(--cj-bg)", position: "relative" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
            ...
            <table className="cmp-table" style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "rgba(212,160,23,0.08)" }}>
                  ...
                  <th style={{ padding: "1rem 1rem", textAlign: "center", color: "#d4a017", fontSize: "0.9375rem", fontWeight: 800, borderLeft: "2px solid rgba(212,160,23,0.35)" }}>NIRI</th>
          ...
          <p style={{ textAlign: "center", color: "#d4a017", fontWeight: 800, fontSize: "1.25rem", marginTop: "2rem" }}>
        ...
        <div style={{ maxWidth: 1140, margin: "0 auto" }}>
          ...
          <div className="grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.5rem" }}>
            ...
            <div className="card-hover lp-card" style={{ background: "linear-gradient(145deg,#1a1508,#0f0c04)", border: "1px solid rgba(212,160,23,0.30)", borderRadius: "1.25rem", overflow: "hidden" }}>
              ...
              <div style={{ padding: "1.25rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.625rem", marginBottom: "1rem" }}>
                  {[["P&L","$2,840","+18%","#4a9e4a"],["Win Rate","67%","up 4%","#d4a017"],["Rank","Gold","Top 12%","#b8860b"]].map(([l,v,s,c]) => (
                    <div key={l} style={{ background: "rgba(212,160,23,0.06)", borderRadius: "0.625rem", padding: "0.625rem 0.5rem", textAlign: "center" }}>
                ...
                <div style={{ background: "rgba(212,160,23,0.06)", borderRadius: "0.75rem", height: 64, overflow: "hidden" }}>
                  <svg viewBox="0 0 260 64" style={{ width: "100%", height: "100%" }} preserveAspectRatio="none">
                    <defs><linearGradient id="g1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#d4a017" stopOpacity="0.25"/><stop offset="100%" stopColor="#d4a017" stopOpacity="0"/></linearGradient></defs>
                    <path d="M0,52 L40,46 L80,38 L120,32 L160,24 L200,18 L260,8" fill="none" stroke="#d4a017" strokeWidth="2"/>
            ...
            <div className="card-hover lp-card" style={{ background: "linear-gradient(145deg,#1a1508,#0f0c04)", border: "1px solid rgba(212,160,23,0.30)", borderRadius: "1.25rem", overflow: "hidden" }}>
              ...
              <div style={{ padding: "1.25rem" }}>
                <div style={{ background: "#0d0d12", borderRadius: "0.75rem", height: 90, marginBottom: "0.875rem", position: "relative", overflow: "hidden" }}>
                  <svg viewBox="0 0 260 90" style={{ width: "100%", height: "100%" }} preserveAspectRatio="none">
                    ...
                    <line x1="0" y1="30" x2="260" y2="30" stroke="#d4a017" strokeWidth="1.5" strokeDasharray="5,3"/>
                  </svg>
                  <div style={{ position: "absolute", top: 4, right: 8, fontSize: "0.625rem", color: "#4a9e4a" }}>ENTRY</div>
                  <div style={{ position: "absolute", top: 22, right: 8, fontSize: "0.625rem", color: "#d4a017" }}>EXIT</div>
                </div>
            ...
            <div className="card-hover lp-card" style={{ background: "linear-gradient(145deg,#1a1508,#0f0c04)", border: "1px solid rgba(212,160,23,0.30)", borderRadius: "1.25rem", overflow: "hidden" }}>
              ...
              <div style={{ padding: "1.25rem" }}>
                {[["CONFIDENT","68%","#4a9e4a",68],["REVENGE","22%","#e05555",22],["FEARFUL","48%","#b8860b",48]].map(([em,pct,col,w]) => (
      ...
      {/* ── STATS ────────────────────────────────────────────────────────────── */}
      <div className="gold-divider" />
      <section style={{ padding: "5rem 1.5rem", background: "radial-gradient(circle at bottom left, rgba(212,160,23,0.10) 0%, transparent 50%), var(--cj-bg)" }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div className="grid-4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1.5rem" }}>
            <div ref={stat1.ref} style={{ textAlign: "center" }}>
              <div style={{ color: "#d4a017", fontWeight: 900, fontSize: "2.5rem", fontVariantNumeric: "tabular-nums" }}>{stat1.val}</div>
            ...
            <div ref={stat2.ref} style={{ textAlign: "center" }}>
              <div style={{ color: "#d4a017", fontWeight: 900, fontSize: "2.5rem", fontVariantNumeric: "tabular-nums" }}>{stat2.val}</div>
            ...
            <div ref={stat3.ref} style={{ textAlign: "center" }}>
              <div style={{ color: "#d4a017", fontWeight: 900, fontSize: "2.5rem" }}>{stat3.val}</div>
            ...
            <div ref={stat4.ref} style={{ textAlign: "center" }}>
              <div style={{ color: "#d4a017", fontWeight: 900, fontSize: "2.5rem" }}>${stat4.val}</div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "3.5rem" }}>
            <div style={{
              ...
              background: "rgba(212,160,23,0.10)", border: "1px solid rgba(212,160,23,0.30)",
              borderRadius: "2rem", padding: "0.375rem 1rem",
              color: "#d4a017", fontSize: "0.8125rem", fontWeight: 700,
          ...
          <div className="grid-4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1.25rem", marginBottom: "3rem" }}>
            {[
              ...
              <div key={s.label} className="lp-card" style={{
                background: "linear-gradient(145deg,rgba(212,160,23,0.08),rgba(245,197,24,0.02))",
                border: "1px solid rgba(212,160,23,0.25)", borderRadius: "1.25rem", padding: "1.75rem",
                textAlign: "center",
              }}>
                <div style={{ color: "#d4a017", fontWeight: 900, fontSize: "1.375rem", marginBottom: "0.375rem", letterSpacing: "-0.02em" }}>{s.val}</div>
          ...
          <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", maxWidth: 800, margin: "3rem auto 0" }}>
            ...
            <div className="lp-card" style={{ background: "linear-gradient(145deg,#1e1a06,#131000)", border: "2px solid #d4a017", borderRadius: "1.375rem", padding: "2.25rem", position: "relative", boxShadow: "0 0 50px rgba(212,160,23,0.12)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
                <p style={{ color: "#d4a017", fontSize: "0.8125rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", margin: 0 }}>Pro</p>
                <span style={{ background: "rgba(212,160,23,0.12)", border: "1px solid rgba(212,160,23,0.35)", color: "#d4a017", fontSize: "0.6875rem", fontWeight: 700, padding: "0.2rem 0.625rem", borderRadius: "2rem", letterSpacing: "0.06em" }}>🌍 Priced for Africa</span>
              </div>
              ...
              return (
                <div key={t.name} ref={tRef} className="fade-up card-hover lp-card" style={{ transitionDelay: `${i * 100}ms`,
                  ...
                  <div style={{ display: "flex", gap: 2, marginBottom: "1rem" }}>
                    {[1,2,3,4,5].map(s => <IcoStar key={s} color="#d4a017" />)}
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                    <div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#d4a017 0%,#f5c842 50%,#d4a017 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#0d0d12", fontSize: "1rem", flexShrink: 0 }}>{t.name[0]}</div>
                    <div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", marginTop: 2 }}>
                        <IcoAward color="#b8860b" size={12} />
                        <span style={{ color: "#b8860b", fontSize: "0.75rem" }}>{t.badge}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* ── DERIV TRADERS ────────────────────────────────────────────────────── */}
      <section style={{ padding: "5rem 1.5rem", background: "radial-gradient(circle at top right, rgba(212,160,23,0.10) 0%, transparent 50%), var(--cj-bg)", position: "relative" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "2rem" }}>
                {["Volatility 10/25/50/75/100", "Boom 500 & 1000", "Crash 500 & 1000", "Step Index", "Jump Indices"].map(t => (
                  <span key={t} style={{
                    background: "rgba(212,160,23,0.08)", border: "1px solid rgba(212,160,23,0.30)",
      ...
      {/* ── FOOTER ───────────────────────────────────────────────────────────── */}
      <footer style={{ background: "var(--cj-surface)", borderTop: "1px solid rgba(212,160,23,0.10)", padding: "3.5rem 1.5rem 2rem" }}>
        <div style={{ maxWidth: 1140, margin: "0 auto" }}>
          <div className="footer-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: "2.5rem", marginBottom: "3rem" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "0.75rem" }}>
                ...
                <div style={{ width: 28, height: 28, borderRadius: "7px", background: "linear-gradient(135deg,#d4a017 0%,#f5c842 50%,#d4a017 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#0d0d12", fontSize: "0.75rem" }}>NI</div>
              ...
              <div style={{ display: "flex", gap: "0.75rem", marginTop: "1.25rem" }}>
                {[
                  ...
                  <a key={label} href="#" aria-label={label} className="lp-social-link" style={{ width: 32, height: 32, borderRadius: "8px", background: "rgba(212,160,23,0.10)", border: "1px solid rgba(212,160,23,0.12)", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none", transition: "background 0.2s" }}
                    onMouseEnter={e => (e.currentTarget.style.background = "rgba(212,160,23,0.30)")}
          ...
          <div style={{ borderTop: "1px solid rgba(212,160,23,0.08)", paddingTop: "1.5rem", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem" }}>
